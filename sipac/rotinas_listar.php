...
//Pegar variaveis de Filtro
 ...
 $arraCampoNome	  = array("Rotina","Módulo","Submódulo","ID");
 $arraCampoValor  = array("rotinas_nome","modulos_nome","submodulos_nome","rotinas_id","rotinas_ordem");
 ...
 $sql = "SELECT
 rotinas_id, rotinas_nome, submodulos_nome, modulos_nome,rotinas_ordem
 FROM rotinas
 INNER JOIN submodulos ON  submodulos.submodulos_id = rotinas.submodulos_id
 INNER JOIN modulos ON modulos.modulos_id = submodulos.modulos_id
 WHERE 1 = 1";
 ...
 if($recordCount1 >0)
 {
  for ($indx = 0; $indx < $reg_pag; $indx++)
  {
    ...
    $lista	.=	"	<td><input type='checkbox' class='checkbox' name='Sels[]' value='". $linha["rotinas_id"]."' /></td>\n";
    $lista	.=	"	<td>". $linha["rotinas_nome"]."</td>\n";
    $lista	.=	"	<td>". $linha["modulos_nome"]."</td>\n";
    $lista	.=	"	<td>". $linha["submodulos_nome"]."</td>\n";
    $lista	.=	"	<td>". $linha["rotinas_ordem"]."</td>\n";
    $lista  .=  $rotinaClass->menu_rotinas_interno($linha["rotinas_id"]);
   	$lista	.=	"</tr>\n";
  }
 }
 else
...
<div id="conteudo">
     ...
     <? $rotinaClass->menu_rotinas_superior(); ?>
       ...
       <td class="ordenarLista" id="i0">Rotina <?= ordenacao(0) ?></td>
       <td class="ordenarLista" id="i1">Módulo <?= ordenacao(1) ?></td>
       <td class="ordenarLista" id="i2">Submódulo <?= ordenacao(2) ?></td>
       <td class="ordenarLista" id="i4">Ordem <?=ordenacao(4) ?></td>
