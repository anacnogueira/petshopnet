<?
 /*--------------------------------------------------------'
   ...
   Página: produtos_mais_comprados.php                    '
---------------------------------------------------------*/
 ...
 $arraCampoValor	= array("produtos_nome","COUNT(pp.pedidos_id)");
 ...
 else
 {
  ...
  $sql = "SELECT
  p.produtos_codigo,produtos_nome, COUNT(pp.pedidos_id) as total
  FROM pedidos_produtos pp
  INNER JOIN produtos p ON p.produtos_codigo = pp.produtos_codigo
  INNER JOIN pedidos pe ON pe.pedidos_id = pp.pedidos_id
  WHERE pedidos_status_id = 3
  GROUP BY p.produtos_id ORDER BY total DESC";
  $result = $conn->sql($sql);
...
<div id="conteudo">
 ...
 <br />
Lista de Produtos Mais Comprados:&nbsp;&nbsp;<?= $resposta?>
