...
//Pegar variaveis de Filtro
 ...
 $arraCampoValor = array("modulos_nome","modulos_id","modulos_ordem");
 ...
 $sql = "SELECT
 modulos_id, modulos_nome,modulos_ordem FROM modulos
 WHERE 1=1";
 ...
 if($recordCount1 >0)
 {
   for ($indx = 0; $indx < $reg_pag; $indx++)
   {
     ...
     $lista	.=	"	<td class='Sels'><input type='checkbox' class='checkbox' name='sels[]' value='". $linha["modulos_id"]."' /></td>\n";
   	 $lista	.=	"	<td>". $linha["modulos_nome"]."</td>\n";
   	 $lista	.=	"	<td>". $linha["modulos_ordem"]."</td>\n";
     $lista  .=  $rotinaClass->menu_rotinas_interno($linha["modulos_id"]);
   	 $lista	.=	"</tr>\n";
   }
 }
 else
...
<div id="conteudo">
     ...
     <? $rotinaClass->menu_rotinas_superior(); ?>
       ...
       <td class="ordenarLista" id="i2">Ordem <?= ordenacao(2) ?></td>
