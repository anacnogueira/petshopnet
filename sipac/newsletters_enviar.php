...
---------------------------------------------------------*/
 ...
 if(isset($_POST["btn_enviar"]) or isset($_POST["btn_global"])) {
  ...
  if(!$_POST["btn_global"] and !empty($arraProdutos))
  ...
  if(mysql_num_rows($result) > 0){
    ...
    while($produto = mysql_fetch_array($result))
    {
      ...
      if(!empty($produto["produtos_imagem"])){
    	...
    	$path            = DIR_PRODUTOS.$name.".thumb".$ext;
        $text            = $produto["produtos_nome"];
      }
      else{
        //Produto sem imagem
        $path  = DIR_IMAGENS."no_image.jpg";
        $text  = "Sem imagem para exibir";
      }
        ...
        $listaProduto .= "  <td><a href='".URL. "produto_info.php?produtos_id=".$produtos_id."&amp;parent_id=".$linha["parent_id"]."&amp;id_cat=".$produto["categorias_id"]."'>\n";
   		$listaProduto .= "    <img src='". URL. $path . "' title='".$text."' alt='".$text."' border='0' /><br />\n";
