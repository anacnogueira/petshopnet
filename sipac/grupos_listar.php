...
//Pegar variaveis de Filtro
 ...
 $arraCampoValor = array("grupos_nome","grupos_id",);
 ...
 $sql = "SELECT
 grupos_id, grupos_nome FROM grupos WHERE 1=1";
 ...
 if($recordCount1 >0)
 {
   for ($indx = 0; $indx < $reg_pag; $indx++)
   {
     ...
     $lista	.=	"	<td class='Sels'><input type='checkbox' class='checkbox' name='Sels[]' value='". $linha["grupos_id"]."' /></td>\n";
   	 $lista	.=	"	<td>". $linha["grupos_nome"]."</td>\n";
     $lista  .=  $rotinaClass->menu_rotinas_interno($linha["grupos_id"]);
   	 $lista	.=	"</tr>\n";
   }
 }
 else
