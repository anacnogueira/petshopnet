...
class Mult_Pag extends ConexaoMysql
{
  // Valores padrão para a navegação dos links
  var $num_pesq_pag;
  var $str_anterior     = "<img src='banco_imagens/btns/arrow_left.gif' border='0' alt='' />";
  var $str_anterior_off = "<img src='banco_imagens/btns/arrow_leftoff.gif' border='0' alt='' />";
  var $str_proxima      = "<img src='banco_imagens/btns/arrow_right.gif' border='0' alt='' />";
  var $str_proxima_off  = "<img src='banco_imagens/btns/arrow_rightoff.gif' border='0' alt='' />";
  ...
  function Executar($sql, $velocidade, $tipo)
  {
    ...
    if ($tipo == "mysql")
    {
      $resultado = ConexaoMysql::sql($total_sql);
      ...
      $resultado = ConexaoMysql::sql($sql); //pesquisa com limites por página
    }
    elseif ($tipo == "pgsql")
    {
  ...
  '|  irá adicionar automaticamente aos links de navegação  dinâmicos.  '|
  '|--------------------------------------------------------------------*/
  function Construir_Url()
  {
    ...
    while (list($chave, $valor) = each($cgi))
      if ($chave != "pagina" and $valor!='')
        $query_string .= "&amp;" . $chave . "=" . urlencode($valor);
  ...
  '|        . "strings" - retornar somente os links 'Próxima' e/ou      '|
  '|        'Anterior'                                                  '|
  '|                                                                    '|
  '|       $mostra_string parâmetro:                                    '|
  '|        . "nao" - mostra 'Próxima' ou 'Anterior' apenas quando      '|
  '|        for necessários                                             '|
  ...
  '|                                                                    '|
  '|      $atual parâmetro:                                             '|
  '|       . a variável da 'pagina' atual das páginas paginadas.        '|
  '|       ex: pagina=1                                                 '|
  '|                                                                    '|
