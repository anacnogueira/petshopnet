...
for($i = 2; $i < 33; $i++)
{
	...
	if($linha == date('d'))
	{
		$listaDias .= "<td class='Hoje listaItem returnData' id ='d".$linha."-".$mes."-".$ano."'>".$linha."</td>\n";
 	}
 else
 {
   $listaDias .= "<td class='listaItem returnData' id='d".$linha."-".$mes."-".$ano."'>".$linha."</td>\n";
 }
...
<table  border="0" cellpadding="0" cellspacing="0" class="tabCalendario">
	...
	<td colspan="4">
	<a href="<?=$pageAtual?>?mes=<?=($mes-1)?>&amp;ano=<?=$ano ?>&amp;object=<?=$object ?>">&laquo;</a>
     <select name="mes" class='submit'>
      <?=fct_Array2_MontarLista($arraMes,$mes) ?>
     </select>
  <a href="<?=$pageAtual?>?mes=<?=($mes+1)?>&amp;ano=<?=$ano ?>&amp;object=<?=$object ?>">&raquo;</a>
  </td>
  <td colspan="3" >
   <a href="<?=$pageAtual?>?mes=<?=$mes?>&amp;ano=<?=($ano-1) ?>&amp;object=<?=$object ?>">&laquo;</a>
   <select name="ano" class='submit'>
     <?= $listaAno ?>
   </select>
   <a href="<?=$pageAtual?>?mes=<?=$mes?>&amp;ano=<?=($ano+1) ?>&amp;object=<?=$object ?>">&raquo;</a>
  </td>
 </tr>
 <tr class='cabecalhoCalendario'>
  <td>Dom</td>
